...
let textOptions = ["Zero Hunger", "No more hunger"];
...
// Map sound to speed
  ...
  for (let i = 0; i < movingTexts.length; i++) {
    ...
    for (let j = mt.history.length - 1; j >= 0; j--) {
      ...
      let alpha = map(j, mt.history.length - 1, 0, 10, movingTextAlpha * movingTextTrailStartAlphaFraction);
      fill(r, g, b, alpha);
